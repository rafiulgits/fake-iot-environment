...
class HomeAutomationSystem(BaseSystem):
    ...
    def setup_living_room_ac(self):
        smartAc = SmartAC(self.mqttClient, {'status': 'ON'}, 7)
        smartAc.set_incomming_command_topic(
            "home/living-room/fan/status/change")
        smartAc.set_status_publishing_topic("home/living-room/fan/status")
        self.devices.append(smartAc)

    def setup_living_room_light(self):
        smartLight = SmartLight(self.mqttClient, {'status': 'ON'}, 10)
        smartLight.set_incomming_command_topic(
            "home/living-room/light/status/change")
